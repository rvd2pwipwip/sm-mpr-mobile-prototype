...
- **Visual ads** — **`docs/mobile/visual-ads-and-user-types.md`**:
  ...
  - **`VisualAdsHtmlSync`** sets **`html[data-visual-ads]`**. **`index.css`**: broad **`--bottom-nav-stack-height`** includes the nav-area ad strip.** **`html[data-catalog-scope="limited"][data-visual-ads]`** sets **`--bottom-nav-stack-height`** to **`--visual-ad-strip-min-height`** so **`MiniPlayer`** and scroll padding reserve only that strip.
...
- **Account body:** **`InfoAccountSection`** (**`5518:74009`**) reads **`useUserType()`** and renders **guest** / **freeStingray** / **freeProvided** / **subscribed** layouts with **`Button`** (**`subscribe-primary`** / **`secondary`**) and **`<a className="btn btn--secondary">`** for external Stingray links (**`src/constants/externalLinks.js`**). **Guest** / **free Stingray**: **Restore purchases** opens **`RestorePurchasePrototypeDialog`** (**Tier A**, no IAP). **Log out** calls **`setUserType("guest")`** for demos.
- **Settings body (Phase 4):** **`InfoSettingsSection`** — **Autoplay** row; **Audio Quality** row (**`aria-haspopup="dialog"`** without tier access) opens **`AppStackedDialog`** upsell (**`9585:70503`**) or expands **`SearchBrowseContentSwitcher`** for **`subscribed`** / **`freeProvided`**. Tabs **Normal / High / Maximum** (`AUDIO_QUALITY_SEGMENTS`) with per‑tier bitrate copy plus **`AUDIO_QUALITY_DATA_CHARGES_NOTE`**; upsell **`AUDIO_QUALITY_UPSELL`**. **Communication preferences** row. **`AppStackedDialog`**: scrim + theme-aware **title band** (light app: dark header; dark app: light header) + body + actions; **`GuestSkipLimitDialog`** uses it; **`--z-stacked-modal`** in **`index.css`**.
- **Info body (Phase 5):** **`InfoHelpSection`** (**`src/components/InfoHelpSection.jsx`**) lists **FAQ** (**`<a href={INFO_FAQ_HREF}>`** from **`src/constants/infoHelpLinks.js`**, **`https://musicsupport.stingray.com/hc/en-us`**; **`externalFaqAnchorProps(INFO_FAQ_HREF)`** adds **`target="_blank"`** + **`rel="noopener noreferrer"`** for **`https`**), **Contact us** / **About** (**`<Link to={INFO_CONTACT_PATH}>`** / **`INFO_ABOUT_PATH`**), chevrons like **Settings**.
...
- **Idea:** **Listening history** for the prototype lives in **`ListenHistoryProvider`** (`src/context/ListenHistoryContext.jsx`): **empty on load**, **in-memory**. **Music:** **`recordMusicChannelListen(id)`** when **`MusicPlayer`** is allowed to play (after **guest preroll** completes or when preroll is skipped). **Podcasts:** **`recordPodcastShowListen(podcastShowId)`** from **`PodcastPlayer`** after preroll and when the user engages (**play** or **progress > ~5%**), once per episode visit. **Radio:** **`recordRadioStationListen(stationId)`** from **`RadioPlayer`** after the same preroll gate as music (**`MusicPlayer`** parity). **Scoped clear:** **`clearHistoryByKind('music' | 'podcast' | 'radio')`** removes only that kind (**`isListenHistoryKind`** guard); **`clearListenHistory()`** still clears the full list (**Listen again More**). **Podcast history Clear** (**`MyLibraryHistoryMore.jsx`**, **`/my-library/history/podcasts`**) also calls **`clearAllEpisodeProgress()`** (**`PodcastUserStateContext`**) so **Continue listening** (in‑memory episode stubs) empties (**limited / broad**, same hub). **Dedup** = move existing id to **newest**; cap with **`LISTEN_HISTORY_MAX_STORED`** (`src/constants/listenHistory.js`); kinds listed in **`LISTEN_HISTORY_KINDS`**. **Home** shows a mixed **Listen again** **`ContentSwimlane`** when `items.length > 0`, with **compact** **`MusicChannelCard`** / **`PodcastCard`** / **`RadioStationCard`** + **ghost** **`ContentTileCard`** fillers up to **`LISTEN_AGAIN_RAIL_SLOT_CAP`** (12). **Limited Browse** (**`LimitedBrowseTaxonomyRails`**) shows **Listen again** **only when** **`items`** for the **current** browse tab **`kind`** (**music** \| **podcasts** \| **radio** ↔ **`podcast`** in storage) are non-empty. **More:** **`/more/listen-again`** → **`ListenAgainMore.jsx`** (grid like **`SwimlaneMore`**), **`ScreenHeader`** **`endSlot`** = text **Clear** (`screen-header__text-btn`). **`renderListenAgainTile`** in **`ListenAgainCard.jsx`** maps **`kind`** **`music` \| `podcast` \| `radio`**. **My Library** (**`LibraryHistoryRail`**) **`filter`** **`items`** by **`segment`** / **`kind`**. **`Home`** keeps a **comment block** above the lane for future **Favorites**.
...
- **Idea:** **Navigation / IA** branches on **`limited` | `broad`**: **broad** means **Home** plus **bottom tabs** plus combined Search/Browse under **`/search/*`**; **limited** means **`LimitedBrowse.jsx`** at **`/`**, **no** primary tab bar, **Search** and **Info** only as **Browse header** actions. **State** stays in **`TerritoryProvider`** (`src/context/TerritoryContext.jsx`). **`musicLineupMode`** drives Search/Browse music mock breadth; **`catalogScope`** derives from **`catalogScopeFromMusicLineup()`** in **`src/constants/catalogScope.js`**. **`data-catalog-scope`** on **`<html>`** drives **`index.css`** (default limited sets **`--bottom-nav-stack-height: 0`**; **limited +** **`html[data-visual-ads]`** reserves the **footer ad strip** height like the **`BottomNav`** ad row).
...
- **Tutorial:** [`SearchBrowseContentSwitcher-thumb-layout-tutorial.md`](Tutorials/SearchBrowseContentSwitcher-thumb-layout-tutorial.md) — step-by-step. **Code:** `src/components/SearchBrowseContentSwitcher.jsx`. **Non-route reuse:** **`mode="local"`** (**`LimitedBrowse.jsx`** Browse pill per **`BROWSE_TABS`** ids; **`Info`** audio quality — see **Info tab** section).
...
## Full player header center (music, radio, podcast)

- **`showUpgradeInFullPlayerHeader`** (`showVisualAds.js`): **Upgrade** in the fullscreen header whenever **`userType !== "subscribed"`** (**`guest`**, **`freeStingray`**, **`freeProvided`**). **`Home`** keeps **`showUpgradeCallToAction`** for **Upgrade vs provider pill** (**`guest`/`freeStingray`** vs **`freeProvided`**); **`HomeHeader.jsx`** unchanged.
- **`PlayerHeaderCenterSlot.jsx`**: **`UpgradeButton`** vs invisible spacer (**`subscribed`** only).
- **`PlayerProvidedBrandRow.jsx`**: **`freeProvided`** only — **`ProviderLogoPair`** (**`HomeHeader.jsx`**, **`HomeHeader.css`**) in **`music-player__provided-brand-row`** (**`MusicPlayer.css`**) between transport and fixed **`VisualAdStrip--player`**. **`Music`/`Radio`**: **`music-player__bottom-player-stack`** wraps controls plus optional brand row; **`Podcast`**: brand row inside **`podcast-player__footer-stack`** after controls.
...
## Share prototype (`SharePrototypeProvider`)

- **Idea:** **`SharePrototypeProvider`** + **`SharePrototypeOverlays`** (**`step`**: **`idle`** \| **`web-share-mock`** \| **`facebook-mock`**) simulate Web Share API without **`navigator.share`**. **`openSharePrototype()`** from **`MusicChannelInfo`**, **`PodcastInfo`**, **`RadioStationInfo`**, and fullscreen **music / podcast / radio** players shows **`dialogShareAPI.png`**; tap advances to **`facebookShare.png`**; tap closes. Assets **`vitePublicHref`** in **`sharePrototypeAssets.js`**; **`--z-share-prototype`** above **`--z-stacked-modal`**.
