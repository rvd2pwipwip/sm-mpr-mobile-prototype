...
- [x] **Catalog scope refactor — Limited Browse taxonomy swimlanes (Phase C)** — **`LimitedBrowseTaxonomyRails`**: optional tab-scoped **Listen again** (Music / Podcasts / Radio rails show only matching history kinds); **Music** | **Podcasts** | **Radio** **`SearchBrowseContentSwitcher`** (**`mode="local"`**) between **`HomeBanner`** and rails filters which taxonomy stack shows; genre / category / format swimlanes; **More** routes match **`/search/browse/...`**. **`docs/Plans/catalog-scope-search-browse-refactor.md`**.
...
*Last updated: 2026-05-13* — **Catalog scope**: **Limited Browse** **Music \| Podcasts \| Radio** pill under banner filters rails; **`LimitedBrowseTaxonomyRails`** + **Listen again** when history exists. **Next:** likes / favorites prepend on limited Browse per plan; **limited Search** result swimlanes polish if needed.
