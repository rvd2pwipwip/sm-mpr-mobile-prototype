...
- [x] **Podcast stack — Phase 1** — **`/podcast/:podcastId`** → **`PodcastInfo.jsx`** (show shell); **`/podcast/:podcastId/play/:episodeId`** → **`PodcastPlayer.jsx`** (full-screen shell: header match music, cover + stub copy); **`getPodcastEpisodeById()`** in **`podcasts.js`**; **`hideBottomNavForPath`** + **`PlaybackContext`** hide mini on **`/podcast/.../play`**; **`Home`** + **`SwimlaneMore`** podcast tiles **`navigate`** to show. Next: Phase 2–5 (library context, EpisodeCard, full player + preroll, **`PlaybackContext` / mini session, Listen again)—see **`docs/Tutorials/Podcasts-implementation-plan.md`**.
...
1. [ ] **Podcast & radio stacks** — Podcast **Phase 1** (**routes + shells**) shipped (see **What we have done**); next: Phase 2+ per **`docs/Tutorials/Podcasts-implementation-plan.md`** (EpisodeCard, library context, **`PlaybackContext`/`MiniPlayer`** session, Listen again). Radio: mirror routes when ready.
