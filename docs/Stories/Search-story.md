...
- **URL / routing** — **Implemented:** browse content-type strip uses **`/search/music`**, **`/search/podcasts`**, **`/search/radio`** ( **`/search`** redirects to **`/search/music`** ) so history and **Back** preserve the active browse tab; search-mode query UX is unchanged.
