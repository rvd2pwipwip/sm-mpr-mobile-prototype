...
I display each category in its corresponding swimlane, either Channels, Artists, Tags, Podcasts, Episodes, or Radio. **Channels, Podcasts, Episodes,** and **Radio** use **catalog media cards**. **Artists** and **Tags** use **square label tiles** (same footprint as Genre browse sub-tiles like “All Pop”): one tile per **artist stub** or per **matching tag label**, respectively. Tags are **distinct chip strings** whose text **contains** the user’s query; tapping a tag opens the usual **two-column grid of channels** carrying that chip (**`/search/more/tags?q=`**). Each swimlane is only displayed when it has results and has a More button where the slice is capped.

Users clicking one of my results swimlane’s More buttons see a vertically scrolling grid: **two columns** for **Channels**, **Artists**, **Tags**, **Podcasts**, and **Radio** (**Tags More** repeats the same tag-label tiles across the grid); **Episodes** stay **single column** episode cards—similar to Home’s More pattern.

When users are on music **Channel Info**, the vibe tags in the chip row match the strings my **Tags** lane uses—tapping a chip jumps straight to the **same** 2-column channel grid (**`/search/more/tags?q=`**). that brings back Browse with an **empty** field and the **same** content-type tab (**Music**, **Podcasts**, or **Radio**) they had before they started searching, with the right territory tiles for that tab. If instead they **tap the Search icon in the bottom bar** while they are already in our house, or they **leave our house** for another main tab and **come back via that same Search icon**, we open on **Music** with an empty field and my brother’s music tiles for the current territory lineup—so that path always lands on the Music front door, no matter which sibling they were visiting before.
...
- **Tags swimlane (spec gap)** — Legacy Figma **Search results** (`61:26534`) does **not** show a **Tags** lane; product intent **does**. When the query **substring**-matches vibe **tag** labels on channels, populate a **Tags** swimlane with **one square label tile per distinct matching tag**. **More** shows the remaining tag tiles; tapping a tile (**Search** or **More**) opens **`/search/more/tags?q=`** (encoded **exact** tag label → 2-column **channel** grid via **`SearchTagsMore.jsx`**).
- **Tag tap from Channel Info** — Tapping a `.music-info__tag` behaves like navigating straight to **`/search/more/tags?q=…`** with that chip’s exact label (**`SearchTagsMore.jsx`**). Equivalent to tapping that tag tile after search (shortcut past typing + Tags rail).
...
- **Reset to Browse** — Behavior depends on **how** the user exits search mode:
  ...
  - **Bottom-nav Search** — **Return** from **Home / My Library** (or any route that is **not** under **`/search`):** **`NavLink`** resolves to the **last** active browse tab (**`sessionStorage`**, **`readStoredBroadSearchBrowseTab`**) so the user returns to **Podcasts** or **Radio** if that was left selected (**prototype** parity with remembering context). Path is **`/search/${music|podcasts|radio}`** (no **`?q=`** in the link — drops an active query when entering from another main tab). **`/search`** (broad) **redirects** to that stored tab, default **`music`** when empty.
  - **Bottom-nav Search (re-tap on Search stack)** — While **`pathname`** is already under **`/search`**, tapping **Search** again **resets** to **Music** with an **empty** field (**`/search/music`** + strip **`?q=`**); storage is updated to **`music`**. **Not** the same as **Clear** when the user meant only to drop the query: re-tap is the **global reset** for the Search tab entry.
...
- **URL / routing** — **Browse** content-type strip uses **`/search/music`**, **`/search/podcasts`**, **`/search/radio`**; **`/search`** (broad) **redirects** to the **last stored** tab (**`readStoredBroadSearchBrowseTab`**) or **`/search/music`** so history and **Back** preserve the active browse tab. While the user has a **live search** (non-empty query), the same routes carry **`?q=`** (debounced, **replace**) so **Back** from **More** grids and media detail returns to **populated** results. **BottomNav Search** uses path-only **`/search/${stored}`** when entering from another main tab; **re-tap** while already on **`/search/*`** forces **`/search/music`** and clears **`q`**.
