...
- **Bottom nav (step 3):** **`catalogScope === broad`** — fourth tab **My Library** (`/my-library`). **`limited`** — fourth tab **Info** (`/info`). **`BottomNav.jsx`** builds the tab list from **`useTerritory().catalogScope`**. **broad:** **`/info`** root redirects to **`/my-library`** (`InfoRootRoute` in **`App.jsx`**); **`/info/about`** and **`/info/contact`** stay available (e.g. links from **My Library**). **limited:** **`Info.jsx`** at **`/info`**. Fourth tab **active** state treats **`/info`** and **`/my-library`** as one stack so **My Library** deep links still highlight the right tab in **limited** mode.
