...
- **Routing change (My Library Phase 1–4):** The bottom tab lands on **`/my-library`** (**`pages/MyLibrary.jsx`**) with **`AppInfoSwimlane`** (gear + **App Info** square tiles: Account and settings, FAQ, Contact us, About). **`/info`** redirects to **`/my-library`**. **`/my-library/account-settings`** (**`MyLibraryAccountSettings.jsx`**) reuses **`InfoAccountSection`** + **`InfoSettingsSection`** in **`InfoCollapsibleSection`** with **both expanded** on load. **`LibraryHistoryRails`** (**`Phase 4`**) lists three **`ContentSwimlane`** sections (**Music History**, **Your Podcast History**, **Your Radio History**) filtered from **`useListenHistory().items`**, always **More** + ghosts to **`LISTEN_AGAIN_RAIL_SLOT_CAP`**. **More** routes: **`/my-library/history/music`**, **`/my-library/history/podcasts`**, **`/my-library/history/radio`** (**`podcasts`** segment → **`listenKind`** **`podcast`** in **`constants/myLibraryHistory.js`**) → **`MyLibraryHistoryMore.jsx`**; header **Clear** calls **`clearHistoryByKind`** then back (same pattern as **Listen again** full list). **`Info.jsx`** is no longer routed; **`InfoHelpSection`** still powers the **Info** collapsible if you revive **`Info.jsx`** locally. Paths **`INFO_CONTACT_PATH`**, **`INFO_ABOUT_PATH`**, **`MY_LIBRARY_ACCOUNT_SETTINGS_PATH`** live in **`infoHelpLinks.js`**.
