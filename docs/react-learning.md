...
- **Routing change (My Library Phase 1–2):** The bottom tab lands on **`/my-library`** (**`pages/MyLibrary.jsx`**) with **`AppInfoSwimlane`** (gear + **App Info** square tiles: Account and settings, FAQ, Contact us, About). **`/info`** redirects to **`/my-library`**. **`/my-library/account-settings`** (**`MyLibraryAccountSettings.jsx`**) reuses **`InfoAccountSection`** + **`InfoSettingsSection`** in **`InfoCollapsibleSection`** with **both expanded** on load. **`Info.jsx`** is no longer routed; **`InfoHelpSection`** still powers the **Info** collapsible if you revive **`Info.jsx`** locally. Paths **`INFO_CONTACT_PATH`**, **`INFO_ABOUT_PATH`**, **`MY_LIBRARY_ACCOUNT_SETTINGS_PATH`** live in **`infoHelpLinks.js`**.
