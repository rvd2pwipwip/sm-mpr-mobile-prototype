...
- **Info body (Phase 5):** **`InfoHelpSection`** (**`src/components/InfoHelpSection.jsx`**) lists **FAQ** (**`<a href={INFO_FAQ_HREF}>`** from **`src/constants/infoHelpLinks.js`**, placeholder **`"#"`** until Figma gives the URL; **`https`** links should use **`target="_blank"`** + **`rel="noopener noreferrer"`**), **Contact us** / **About** (**`<Link to="/info/contact">`** / **`/info/about`**), chevrons like **Settings**.
...
- **Where to preview podcast/radio UI:** **`/`** podcasts swimlane and real **`/podcast/...`** / **`/radio/...`** play routes. (Legacy **`Info`** tab **`PlaybackContext`** demo buttons were removed when the Info route shells shipped.)
