...
} from "../constants/guestMusicSkips.js";
import { usesGuestMusicSkipCap } from "@sm-mpr/shared/utils/userTierRules.js";
...
export function GuestMusicSkipProvider({ children }) {
  ...
  return (
    <GuestMusicSkipInnerProvider key={userType}>
      {children}
    </GuestMusicSkipInnerProvider>
  );
}
...
function GuestMusicSkipInnerProvider({ children }) {
  ...
  const consumeGuestMusicSkip = useCallback(() => {
    ...
    const next = [...pruned, now + GUEST_MUSIC_SKIP_RECOVERY_MS].sort(
      (a, b) => a - b,
    );
  ...
  return (
    <GuestMusicSkipContext.Provider value={value}>
      {children}
    </GuestMusicSkipContext.Provider>
  );
}
...
export function useGuestMusicSkips() {
  ...
  if (!ctx) {
    throw new Error(
      "useGuestMusicSkips must be used within GuestMusicSkipProvider",
    );
  }
  return ctx;
}
